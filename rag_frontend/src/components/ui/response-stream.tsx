...
function ResponseStream({
  textStream, mode = "typewriter", speed = 20, className = "", onComplete,
  as = "div", fadeDuration, segmentDelay, characterChunkSize,
}: ResponseStreamProps) {
  ...
  const fadeStyle = `
    ...
    .fade-segment { display: inline-block; opacity: 0; animation: fadeIn ${getFadeDuration()}ms ease-out forwards; }
            ...
            {segments.map((segment, idx) => {
              ...
              return (
                <span
                  ...
                  onAnimationEnd={idx === segments.length - 1 ? handleLastSegmentAnimationEnd : undefined}
                >
                  {segment.text}
                </span>
              )
